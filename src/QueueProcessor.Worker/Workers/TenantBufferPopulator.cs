...
public sealed class TenantBufferPopulator : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        await foreach (var msg in _queueClient.ReadMessagesAsync(stoppingToken))
        {
            ...
            try
            {
                var enqueueStatus = await _buffer.EnqueueAsync(buffered, stoppingToken);
                switch (enqueueStatus)
                {
                    case EnqueueStatus.Enqueued:
                        _logger.LogInformation("Enqueued message {MessageId} for tenant {TenantId}", msg.Id, tenantId);
                        break;
                    case EnqueueStatus.NotEnqueuedAccept:
                        _logger.LogWarning("Accepting message {MessageId} for tenant {TenantId} due to its buffer capacity", msg.Id, tenantId);
                        await _queueClient.AcceptMessageAsync(msg, stoppingToken);
                        break;
                    case EnqueueStatus.NotEnqueuedSkip:
                        _logger.LogWarning("Skipping message {MessageId} for tenant {TenantId} due to its buffer capacity", msg.Id, tenantId);
                        await _queueClient.SkipMessageAsync(msg, stoppingToken);
                        break;
                }
            }
